"""
Descarga PDFs de minutos PRAC → data/EMA PRAC/minutes/ (OPTIMIZADO CON PARALELO)
"""
...
SEARCH_URL = (
    ...
    "&doc_type_id_b%5B33607%5D=33607&doc_type_op_b=or"
    "&committee_id_b%5B100010%5D=100010&committee_op_b=or"
    "&categories_id_b%5B83%5D=83&categories_op_b=or"
    "&publishing_date_b%5Bmin%5D%5Bdate%5D="
    "&publishing_date_b%5Bmax%5D%5Bdate%5D="
)

OUTPUT = Path(__file__).parent.parent / "data" / "EMA PRAC" / "minutes"
...
def process_page(page_num: int, session: requests.Session) -> tuple[int, int]:
    ...
    url = f"{SEARCH_URL}&page=%2C1%2C{page_num}"
    try:
    ...
    pdf_links = [u for u in all_links if safe_filename(u).startswith("minutes-")]
    
    if not pdf_links:
        return (0, 0)
    
    downloaded = skipped = 0
    print(f"--- Página {page_num + 1}: {len(pdf_links)} minutos")
...
def check_and_download(session: requests.Session, start_page: int, max_pages: int | None) -> None:
    ...
    for _ in range(max_attempts):
        ...
        try:
            ...
            # Ajuste adaptativo: si hubo 429, duplicar sleep; si no, bajar un poco
            if hit_429:
        ...
        pdf_links = [u for u in all_links if safe_filename(u).startswith("minutes-")]
        if not pdf_links:
            print(f"  Página {page_num + 1}: sin minutos. Fin del listado.")
            break

        missing = [u for u in pdf_links if not (OUTPUT / safe_filename(u)).exists()]

        if missing:
            print(f"--- Página {page_num + 1}: {len(missing)}/{len(pdf_links)} minutos faltan → descargando...")
            ...
            print(f"    Página {page_num + 1}: {len(pdf_links)} minutos — todos ya descargados.")
...
def main() -> None:
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            pdf_count = len([u for u in all_links if safe_filename(u).startswith("minutes-")])
            if pdf_count == 0:
                print(f"  Página {page_num + 1}: Sin minutos. Fin del listado.")
                break
            print(f"  Página {page_num + 1}: {pdf_count} minutos encontrados")
